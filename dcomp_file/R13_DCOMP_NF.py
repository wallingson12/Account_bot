...
def format_field_13(value, start, end, field_type):
    length = end - start + 1
    if pd.isna(value):
...
def generate_line(row):
    line = ''
    line += format_field_13(row['Tipo'], 1, 3, 'X')  # Tipo
    line += format_field_13(row['CNPJ do Declarante'], 4, 17, 'CNPJ')  # CNPJ do Declarante
    line += ' ' * 14  # 14 espaços vazios
    line += format_field_13(row['CNPJ do Estabelecimento Detentor do Crédito'], 32, 45, 'CNPJ')  # CNPJ do Estabelecimento Detentor do Crédito
    line += format_field_13(row['CNPJ do Emitente'], 46, 59, 'CNPJ')  # CNPJ do Emitente
    line += format_field_13(row['Nº da Nota Fiscal'], 60, 68, 'XN')  # Nº da Nota Fiscal
    line += format_field_13(row['Série/Subsérie'], 69, 71, 'X')  # Série/Subsérie
    line += format_field_13(row['Data de Emissão'], 72, 79, 'DATA')  # Data de Emissão
    line += format_field_13(row['Data de Entrada no Estabelecimento Detentor do Crédito'], 80, 87, 'DATA')  # Data de Entrada
    line += format_field_13(row['CFOP'], 88, 91, 'CFOP')  # CFOP
    line += format_field_13(row['Valor Total'], 92, 105, 'R+')  # Valor Total
    line += format_field_13(row['Valor do IPI Destacado'], 106, 119, 'R+')  # Valor do IPI Destacado
    line += format_field_13(row['Valor do IPI Creditado no Livro RAIPI'], 120, 133, 'R+')  # Valor do IPI Creditado
    line += '\n'  # Adiciona uma nova linha

    return line

# Função para processar o arquivo Excel e gerar o arquivo TXT
def process_file_13(file_path):
